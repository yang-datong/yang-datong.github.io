...
$\therefore n \rightarrow \infty , \frac{1}{n}$接近与0
...
$\therefore n \rightarrow\infty , \frac{n}{n+1}$ 接近与1

* 此时 $y_n$ **收敛**

> $exist => \{y_n\}, y_n = (-1)^n$

$\because \lim\limits_{n\rightarrow\infty}y_n = \lim\limits_{n\rightarrow\infty} (-1)^n  =>$此时不存在唯一一个常数（可为1、-1）
...
* $x \rightarrow x_0 时f(x)的极限, (此时x_0是常数, x是动点)$
    - $x\rightarrow x_0^-$ => 在x轴中 : x位于`负侧`无限接近 $x_0$,此时 $x<x_0$
    - $x\rightarrow x_0^+$ => 在x轴中 : x位于`正侧`无限接近 $x_0$,此时 $x>x_0$
    - $x\rightarrow x_0$  => 在x轴中 : $x$从点$x_0$的`两侧`无限接近 $x_0$
    - $x无限接近与x_0但永不会等于x_0$
        <br>
...
<font color="red"> 置于f(x) 无穷大时=>使分子不为0, 分母为0

置于f(x) 无穷小时=>使分子为0, 分母不为0</font>
...
##### 重要极限=>$\lim\limits_{x\rightarrow 0}\frac{sinx}{x}= 1$
...
##### 重要极限=>$\lim\limits_{x\rightarrow \infty}(1+\frac{1}{x})^x = e$
...
* $as => \lim\limits_{x\rightarrow 2}\frac{x^2-4}{x-2} 此时存在极限值, 但此时x=2时f(x) 无定义(分母为0)$
    ...
    - 得出结论1: $f(x) 在x_0有定义$ (这里需要区分一下函数值和极限值的区别)
...
```mermaid
graph LR
classDef main fill:#383838,stroke:#fff,stroke-width:1px,color:#fff
linkStyle default stroke-width:4px,fill:none,stroke:#9CCDCB
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
class 1,2,3,4,5 main
```

##### 注意：当极限无穷时，这里也是等同于不存在的
...
> $\displaystyle{exitst => f(x), g(x) 满足：}$
> $\displaystyle{1. 点x_0在某去心领域内可导、且g'(x) \neq  0}$
> $\displaystyle{2. \lim\limits_{x\rightarrow x_0}f(x) = 0 , \lim\limits_{x\rightarrow x_0}g(x) = 0 }$
> $\displaystyle{3. \lim\limits_{x\rightarrow x_0}\frac{f'(x)}{g'(x)} = A(或\infty )}$
...
极限值则为如上图的$\frac{F}{G}$的比值 => 洛必达法则表示为在点B处$\frac{F}{G} = \frac{f(x)倾斜的斜率}{g(x)倾斜的斜率}$这里的倾斜斜率就是它的微分，表达式记为:$\frac{f'}{g'}$
...
& 2)\quad 由上证明和极限定义可得不等式=> \bigg|\frac{1-g(c) / g(x)}{1-f(c) / f(x)} - 1\bigg| < \frac{\epsilon}{|M|+\epsilon}\\

& => \frac{f(x)}{g(x)} = \frac{1-g(c) / g(x)}{1 - f(c) / f(x)} \cdot \frac{f'(\xi_x)}{g'(\xi_x)} \\
...
<br><br><br><br><br><br><br><br><br><br><br><br><br><brr><br><br><br><br><br>
### 奇、偶函数
* **奇函数**
...
偶函数关于y轴对称，常见函数有$\displaystyle{|x|,\cos{x},\cosh{x}}$
